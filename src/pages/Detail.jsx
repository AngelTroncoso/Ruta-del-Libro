...
export default function DetailPage({ session }) {
  ...
  useEffect(() => {
    if (content) {
      setSelectedCover(content.cover);
    }
  }, [content]);
  ...
  useEffect(() => {
    ...
    fetch('/api/generate-experience', {
      ...
      .catch(err => {
        ...
        setError(err.message || "No se pudo conectar con la IA de Gemini. Asegúrate de que el backend esté corriendo.");
        setLoading(false);
      });
    ...
    <div className="detail-page" style={{ paddingBottom: '100px' }}>
      ...
      <div style={{ display: 'flex', gap: '32px', marginBottom: '48px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', alignItems: 'center' }}>
          <img 
            src={selectedCover || content.cover} 
            alt={content.title} 
            style={{ width: '250px', height: '250px', objectFit: 'cover', borderRadius: '12px', boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }} 
          />
          {content.alt_covers && content.alt_covers.length > 0 && (
            <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
              {content.alt_covers.map((c, i) => (
                <button
                  key={i}
                  onClick={() => setSelectedCover(c)}
                  style={{
                    border: (selectedCover || content.cover) === c ? '2px solid var(--accent)' : '2px solid rgba(255,255,255,0.2)',
                  ...
                  title={c.includes('1971') ? 'Edición Original 1971' : 'Edición Moderna 2005'}
                >
                  <img
                    src={c}
                    alt={`Portada ${i + 1}`}
